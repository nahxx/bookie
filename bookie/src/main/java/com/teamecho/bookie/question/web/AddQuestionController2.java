...
public class AddQuestionController2 {
	...
	public String addQuestions(AddQuestionsCommand command, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// 지문, 문제 분리
		List<String> mList = new ArrayList<String>(); // 지문 담을 리스트
		List<String> qList = new ArrayList<String>(); // 문제 담을 리스트
		List<String> textList = new ArrayList<String>(); // 지문, 문제 구분자 제거 후 텍스트 담을 리스트
		...
		if(qList.size() == 0 || qList.size() == 1) { // 한문제인 경우
